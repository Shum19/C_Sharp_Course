...
int [] InputDate ()
{
    ...
    while (i < size)
    {   // первый if для того чтобы вывести строку что это число A и ввести 1-й элемнет массива
        if (i == 0)
        {
            Console.Write("Write digit of A: ");
            arr [i] = int.Parse(Console.ReadLine());
        }
        //else для того чтобы вывести строку что это число B и ввести 2-й элемнет массива
        else 
        {
...
// А элементы будут те которые вводятся через консоль
int [] test = InputDate();
// Функция возведения в степень которая использует элементы массива, который ииницилизированы выше функцией
// InputDate()
