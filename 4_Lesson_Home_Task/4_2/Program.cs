...
// Функция складывает чила по оотделности
int SumOfDigits (int digit)
{
    int result = 0;
    // пустая строка для того чтобы вывести по отдельности цифры введденые числа
    ...
    for (int i = digit; i > 0; i = i/10)
    {
        ...
        // данная строка получает только посдеднею цифру от введеного числа
        digit = i%10;
